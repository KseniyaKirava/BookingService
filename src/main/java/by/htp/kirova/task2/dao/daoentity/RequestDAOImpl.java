 ...
 * @since Sept 24, 2018
 */
public class RequestDAOImpl implements BookingDAO<Request> {
    ...
    public boolean create(Request request) throws DAOException {
        ...
        try {
            ...
            ps.setInt(1, request.getRoomCapacity());
            ps.setLong(2, request.getCheckinDate());
            ps.setLong(3, request.getCheckoutDate());
            ps.setString(4, request.getRoomClass());
            ps.setBoolean(5,request.isEnabled());
            ps.setString(6, request.getUsersUsername());
